...
export function createTokenSheet(cssText) {
    const sheet = new CSSStyleSheet();
    sheet.replaceSync(cssText);
    return sheet;
}
/** Shared primitive tokens — identical in light and dark themes. */
const PRIMITIVES_CSS = `  /* --- Gray scale --- */
...
const LIGHT_SEMANTIC_CSS = `  /* --- Surface (light) --- */
...
const DARK_SEMANTIC_CSS = `  /* --- Surface (dark) --- */
...
export const lightTokenSheet = createTokenSheet(LIGHT_TOKENS_CSS);
/** Pre-built dark token sheet singleton. */
export const darkTokenSheet = createTokenSheet(DARK_TOKENS_CSS);
/**
 * Merge multiple stylesheets into a new sheet, preserving cascade order.
 * Later sheets override earlier ones.
 */
export function mergeTokenSheets(...sheets) {
    const parts = [];
...
//# sourceMappingURL=token-sheets.js.map